...
public class Player1 : PlayerController
{
    ...
    protected override void ShootBullet()//ミサイル発射
    {
        if (gunCnt >= gunSpan)
        {
    ...
    protected override void Final_Skill()//必殺技
    {
        if (curFullCharge_Gauge >= maxFullCharge_Gauge)
        {
            ...
            for (int i = 0; i < 12; i++)
            {
                Quaternion q = Quaternion.Euler(0, i * 30f, 0);
                Destroy(Instantiate(missile_Prefab, this.transform.position, missilePos.rotation * q), 3.5f);
            }
        }
    }
    //-----------------------------------------------------------------------------------------------------------------------
